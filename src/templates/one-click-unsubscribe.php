...
$srk_param = ! empty( $_POST['srk'] ) ? sanitize_text_field( wp_unslash( $_POST['srk'] ) )  : ( ! empty( $_GET['srk'] ) ?  sanitize_text_field( wp_unslash( $_GET['srk'] ) )  : '' );
$manager_link .= "srek=" . $srk . "&srk=" . $srk_param . "&amp;srsrc=e";
ob_start();

if ( is_object( $post ) ) {

	$message = html_entity_decode( stripslashes( get_option( 'subscribe_reloaded_oneclick_text' ) ), ENT_QUOTES | ENT_HTML5, 'UTF-8' );
